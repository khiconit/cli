...
} from "@shopify/cli-kit/node/api/admin";
...
class TokenRefreshError extends AbortError {
  constructor() {
    super(
      "Failed to refresh credentials. Check that your app is installed, and try again.",
    );
  }
}
...
export function setupGraphiQLServer({
  ...
  const faviconPath =
    require.resolve("@shopify/app/assets/graphiql/favicon.ico");
  ...
  app.get("/graphiql", async (req, res) => {
    ...
    function decodeQueryString(input: string | undefined) {
      return input
        ? decodeURIComponent(input).replace(/\n/g, "\\n")
        : undefined;
    }
  ...
  return app.listen(port, "0.0.0.0", () =>
    stdout.write(`GraphiQL server started on port ${port}`),
  );
}
